...
def size_tires(ac: Aircraft, update_ac = False):
    ...
    tire_to_speed = 1.1 * 100 # [Kts]
    tire_app_speed = 1.2 * 100 # [Kts]
    ...
    tire_data = pd.read_csv('C:/Users\elzer\PycharmProjects\dse-g16-stol\lookups/type3_tires.csv')
    ...
    print(f'MLG tire option: {tire_options_mlg}')
    print(f'NLG tire option: {tire_options_nlg}')
...
def tire_location(ac: Aircraft, update_ac = False):
    ...
    Z_cg = 0.55 * 2.15    # [m] ASSUME 0.55 FUSELAGE HEIGHT
    X_tcone = 8.83 # [m]
    Z_tcone = 1.2                  # [m]
    Z_prop = 1.8                    # [m]
    Y_prop = 2.295      # [m] for the outermost wing-mounted propeller
    X_prop = 2.5                  # [m] nose to prop
    bw = ac.wing.span              # [m]
    Z_tip = 2.15                    # [m]
    X_tip = 2.5                    # [m]
    a = ac.landing_gear.a
    ...
    if gear_type == 'tricycle':
        ...
        print(f"X_nlg_fwd = {X_nlg_fwd_lim}")
        print(f"X_nlg_aft = {X_nlg_aft_lim}")
    ...
    if gear_type == 'taildragger':
        ...
        print(l_nlg_aft ** 2 * np.tan(turnover) ** 2 / ((Z_cg - Z_mlg)) ** 2)
        print(l_nlg_aft)
        Y_mlg_req2 = Y_prop - (Z_prop - Z_mlg) / np.tan(bank)
        # no req3
        delta_Z_mlg = (a * s + (D_tire - D_rim) / 2)
    ...
    if update_ac:
        ...
        return [X_nlg, Y_nlg, Z_nlg], [X_mlg, Y_mlg, Z_mlg]


reqs = loader.load('C:/Users\elzer\PycharmProjects\dse-g16-stol\yamls/reqs.yaml', ac.Requirements)
weights = loader.load('C:/Users\elzer\PycharmProjects\dse-g16-stol\yamls\weights.yaml', ac.Weights)
lndg_gear = loader.load('C:/Users\elzer\PycharmProjects\dse-g16-stol\concepts\landing_gear_tricycle.yaml', ac.Landing_gear)

eng = 0
fuse = 0
mission = 0
wing = loader.load('C:/Users\elzer\PycharmProjects\dse-g16-stol\yamls\wing.yaml', ac.Wing)

name = 'hi'

aircraft = Aircraft(requirements = reqs,
                        weights = weights,
                        landing_gear=lndg_gear,
                    engine=eng,
                    fuselage=fuse,
                    mission=mission,
                    wing=wing,
...
size_tires(aircraft, update_ac=True)

tire_location(aircraft)