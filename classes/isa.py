...
class Atmosphere:
    def __init__(self, height, delta_T=0.0):
        self.height = np.asarray(height, dtype=float)
        self.delta_T = delta_T

        # Validate (works for scalar or array)
        if np.any((self.height < 0) | (self.height > 10000)):
            raise ValueError("Height must be between 0m and 10,000m.")

        # Geopotential height
        H = CONSTS.r * self.height / (CONSTS.r + self.height)

        # ISA temperature
        self.temp_isa = CONSTS.T0 + CONSTS.L * H
        self.temp = self.temp_isa + self.delta_T

        # Pressure (ISA barometric formula)
