...
def R_wf(M, R_N_fus):
    ...
    if M == 0.25:
        return interp_value(read_csv('lookups/roskam_p6_fig_4_1_rwf.csv'), R_N_fus,
                            x_col='Fuselage Reynolds Number (M = 0.25)',
                            y_col='Wing-Fuse Interference Factor (M = 0.25)', log_x=True)
    else:
        return interp_value(read_csv('lookups/roskam_p6_fig_4_1_rwf_2.csv'), R_N_fus,
                            x_col='Fuselage Reynolds Number (M = 0.4)', y_col='Wing-Fuse Interference Factor (M = 0.4)',
                            log_x=True)


def R_LS(sweep_t_c_max_deg):
    return interp_value(read_csv('lookups/roskam_p6_fig_4_2_rls.csv'), np.cos(np.deg2rad(sweep_t_c_max_deg)),
                        'cos(quarter chord)', 'lifting surface correction', log_x=False)
...
def S_wet_fus(l_nosecone, l_tot, l_tailcone, d_max):
    ...
    return np.pi * d_max / 4 * (1 / (3 * l_nosecone ** 2) * (
                (4 * l_nosecone ** 2 + d_max ** 2 / 4) ** 1.5 - d_max ** 3 / 8) - d_max + 4 * l2 + 2 * np.sqrt(
        l_tailcone ** 2 + d_max ** 2 / 4))
...
def D_CD_flap_stuff(ac: Aircraft, flap_type, t_c_max, flap_deflection, wing_drag, flight_condition, cdash_c):
    ...
    if flap_type == 'split':  # 'split' or 'plain' or 'slotted' 'fowler' or 'krueger'
        ...
        D_CD_flap_stuff = interp_value(read_csv(f'lookups/t_c_0.{fd}.csv'), cf_c, f'cf/c ({flight_condition})',
                                       f'dCdp ({flight_condition})', log_x=False)
        ...
        D_CD_flap_stuff = interp_value(read_csv('lookups/cf_c_comb2.csv'), flap_deflection, 'df', f'dCdp (cf={fd}0)',
                                       log_x=False)
        ...
        D_CD_flap_stuff = interp_value(read_csv('lookups/roskam_p6_fig_4_48.csv'), flap_deflection, f'df(cf/c={fd})',
                                       f'dCdp(cf/c={fd})', log_x=False)
...
def CD0_fuselage(M, R_N_fus, l_fus, d_fus_max, l_nosecone, l_tailcone, S, d_b, S_fus_max,
                 surface_type: str = 'fuselage'):
    Rwf = R_wf(M, R_N_fus)
    Cf = C_f(R_N_fus, M)
...
def CD0_flap_profile(ac: Aircraft, flap_type, t_c_max, flap_deflection, wing_drag, flight_condition, slat_or_flap: str,
                     cdash_c):
...
def CD0_gear(x_nlg, height_nlg, n_main_wheels, n_nose_wheels, nlg_tire_diameter, nlg_tire_width, mlg_tire_diameter,
             mlg_tire_width, S, fairing_type: str = 'C', gear_type_roskam: int = 2):
...
def CD0(ac: Aircraft,
    ...
    # Fuselage
    fuselage = CD0_fuselage(M, R_N_fuselage, l_f, d_fus_max, f.nose_cone_length, f.tail_cone_length, S, d_base,
                            f.max_cross_section_area, surface_type='fuselage')

    # HT
    h_tail = CD0_wing(M, R_N_fuselage,
                      sweep_t_c_max_deg=sweep_at_x_c_deg(ht['sweep_LE_deg'], ht['c_r_h'], ht['b_h'], ht['taper_ratio'],
                                                         ht['loc_t_c_max']),
                      R_N_w=R_N(density, speed,
                                exposed_vt_area_and_mgc(S_base, l_f, f.tail_cone_length, ht['x_le'], ht['area'] / 2,
                                                        ht['b_h'] / 2, ht['taper_ratio'], ht['c_r_h'], h_f)[1], mu),
                      t_c_max=ht['t_c_max'],
                      x_c_t_c_max=ht['loc_t_c_max'],
                      S_exp=exposed_vt_area_and_mgc(S_base, l_f, f.tail_cone_length, ht['x_le'], ht['area'] / 2,
                                                    ht['b_h'] / 2, ht['taper_ratio'], ht['c_r_h'], h_f)[0],
                      surface_type='ht',
                      S=ht['area'])

    # VT
    v_tail = CD0_wing(M, R_N_fuselage,
                      sweep_t_c_max_deg=sweep_at_x_c_deg(vt['sweep_LE_deg'], vt['c_r_v'], vt['b_v'], vt['taper_ratio'],
                                                         vt['loc_t_c_max']),
                      R_N_w=R_N(density, speed,
                                exposed_vt_area_and_mgc(S_base, l_f, f.tail_cone_length, vt['x_le'], vt['area'],
                                                        vt['b_v'], vt['taper_ratio'], vt['c_r_v'], h_f)[1], mu),
                      t_c_max=vt['t_c_max'],
                      x_c_t_c_max=vt['loc_t_c_max'],
                      S_exp=exposed_vt_area_and_mgc(S_base, l_f, f.tail_cone_length, vt['x_le'], vt['area'], vt['b_v'],
                                                    vt['taper_ratio'], vt['c_r_v'], h_f)[0],
                      surface_type='vt',
                      S=vt['area'])
    ...
    nacelle_interference = CD0_wing_nacelle_interference(i_n_deg=eng.i_n,
                                                         ...
                                                         chord_at_nacelle=chord_at_y_span(c_r, taper,
                                                                                          y=eng.eng_y_pos_fuselage + b_f / 2,
                                                                                          b=b),
...
def C_D_L(ac: Aircraft,
    ...
    if ac.wing.sweep == 0:
        ...
        e = 4.61 * (1 - 0.045 * A_eff ** 0.68) * (
            np.cos(np.deg2rad(LE_sweep_deg(ac.wing.sweep, c_r, b, taper)))) ** 0.15 - 3.1
